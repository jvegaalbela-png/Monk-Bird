(function () {
  ...
  const countEl = $("count");
  const fShorter = $("f-shorter");
  const fHenderson = $("f-henderson");

  let last = null;

  function pool() {
    return COMPOSITIONS.filter((c) => {
      if (c.composer === "shorter" && !fShorter.checked) return false;
      if (c.composer === "henderson" && !fHenderson.checked) return false;
      return true;
    });
  }
  ...
  function render(c) {
    const label = c.composer === "shorter" ? "Wayne Shorter" : "Joe Henderson";
    ...
    cover.classList.toggle("is-shorter", c.composer === "shorter");
    cover.classList.toggle("is-henderson", c.composer === "henderson");
  ...
  fShorter.addEventListener("change", updateCount);
  fHenderson.addEventListener("change", updateCount);
